...
export const useWeighIns = () => {
  ...
  const { mutateAsync: addWeighIn } = useMutation({
    mutationFn: async ({ dbEntry, existingLogId }) => {
    ...
    onError: (err, { dbEntry, isUpdate, existingLogId }, context) => {
    ...
    onSuccess: (result, variables, context) => {
      if (!result.isUpdate) {
        ...
        queryClient.setQueryData(['weighIns'], (old = []) => 
          old.map(log => log.id === context.newLogEntry.id ? { ...log, id: result.data.id } : log)
        );
      }
    }
  });
  ...
  const { mutateAsync: updateWeighIn } = useMutation({
    mutationFn: async ({ updates, id, nextLogUpdates, nextLogId }) => {
      if (!navigator.onLine) throw new Error('NetworkError');
    ...
    onMutate: async ({ updatedLogs }) => {
      await queryClient.cancelQueries({ queryKey: ['weighIns'] });
      const previousLogs = queryClient.getQueryData(['weighIns']);
      queryClient.setQueryData(['weighIns'], updatedLogs);
      return { previousLogs };
    },
    onError: (err, { updates, id, nextLogUpdates, nextLogId }, context) => {
      if (err.message === 'Failed to fetch' || err.message.includes('NetworkError')) {
  ...
  const { mutateAsync: removeWeighIn } = useMutation({
    mutationFn: async ({ id, nextLogUpdates, nextLogId }) => {
      if (!navigator.onLine) throw new Error('NetworkError');
    ...
    onError: (err, { id, nextLogUpdates, nextLogId }, context) => {
      if (err.message === 'Failed to fetch' || err.message.includes('NetworkError')) {
